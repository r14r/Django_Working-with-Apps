...
class RegistrationViewTest(TestCase):
    def test_register_page_loads(self):
        response = self.client.get(reverse('app_auth:register'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Registrieren')

    def test_register_creates_unverified_user(self):
        response = self.client.post(reverse('app_auth:register'), {
            'username': 'testuser',
    ...
    def test_register_sends_verification_email(self):
        ...
        self.client.post(reverse('app_auth:register'), {
            'username': 'mailuser',
...
class EmailVerificationTest(TestCase):
    ...
    def test_verify_email_link_activates_account(self):
        ...
        response = self.client.get(reverse('app_auth:verify_email', kwargs={'token': str(token)}))
        self.assertRedirects(response, reverse('app_auth:login'))
    ...
    def test_verify_email_invalid_token_returns_404(self):
        ...
        response = self.client.get(reverse('app_auth:verify_email', kwargs={'token': str(fake_token)}))
        self.assertEqual(response.status_code, 404)

    def test_login_blocked_for_unverified_user(self):
        response = self.client.post(reverse('app_auth:login'), {
            'username': 'verifyuser',
            'password': 'S3cur3Pa$$word!',
        })
        self.assertRedirects(response, reverse('app_auth:verify_email_pending'))
    ...
    def test_resend_verification_email_sends_mail(self):
        from django.core import mail
        response = self.client.post(reverse('app_auth:resend_verification'), {
            'email': 'verify@example.com',
        })
        self.assertRedirects(response, reverse('app_auth:verify_email_pending'))
        self.assertEqual(len(mail.outbox), 1)
